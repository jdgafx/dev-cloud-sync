...
export function createJobsRouter(rclone: RcloneService, io: any): Router {
    ...
    router.post('/:id/run', validate(idParamSchema, 'params'), async (req, res, next) => {
        try {
            const success = await rclone.runJobNow(req.params.id!);
            if (!success) {
                throw new BadRequestError('Could not start job. It may already be running or does not exist.');
            }
            io.emit('jobs:update', rclone.getJobs());
